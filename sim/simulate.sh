#!/usr/bin/env bash
# Simulates the report's GCP VM deployment locally with Docker (report §3.1/§6.4).
#
# Mapping:
...
#
# Usage:
#   sim/simulate.sh           build, run, smoke-test (default)
#   sim/simulate.sh --down    stop + remove the container
#   sim/simulate.sh --clean   --down + delete state, workspaces, and image
#
# Env:
...
echo "Teardown:  sim/simulate.sh --down   (or --clean to wipe everything)"
