...
**Run:** `sim/ui/start.sh` → open the printed URL → paste the printed token. The backend re-reads `sim/run/token` on every request, so `simulate.sh` redeploys need no UI restart.
