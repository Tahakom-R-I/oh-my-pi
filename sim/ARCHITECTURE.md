...
sequenceDiagram
  ...
  T->>T: load token (sim/run/token), session pointer (~/.local/state/…)
...
| `sim/server.ts` | API wrapper (~140 lines Bun/TS) | Bearer-token auth, routing, session map, per-session busy lock (409), SSE bridge from `AgentSession` events, session create/resume, CORS, SIGTERM disposal. |
| `sim/package.json` | image build | Pins the npm dependency used inside the container. |
| `sim/omp-remote` | client (python3 + curl, no other deps) | Maintains the session pointer, auto-resumes after wrapper restarts, and streams the **full agent view by default**: model thinking, tool calls with live output, todo lists, retries, compaction, notices, final answer (`--quiet` = text only, `--raw` = raw SSE). Session commands: `--new/--steer/--abort/--status/--sync`; modes: `--git`, `--cwd`. |
| `sim/run/token` | secret | Bearer token (re-generated by each `simulate.sh` run; the client re-reads it every invocation). |
| `sim/run/state/` → `/state` (via `HOME`) | persistent state | omp's entire config root: `agent/agent.db` (credential vault, SQLite), `agent/config.yml` (model roles: `zai/glm-5.3-flash`), `agent/models.db` (catalog cache), `agent/sessions/<encoded-cwd>/*.jsonl` (transcripts), `logs/`. |
| `sim/run/workspaces/` → `/workspaces` | workspaces | One directory per session (`/workspaces/<uuid>`); where the agent's tools operate. Optional: `LOCAL_PROJECT=<dir>` bind-mounts a real host directory at `/workspaces/project` so agent edits hit live host files. |
| `sim/run/git-bridge/` → `/git-bridge` | git bridge | Bare repo shared with the container; `--git` sessions clone it at `/workspaces/git/project` (§11). |
...
| `sim/git-bridge` | host helper | `seed/push/pull/log/reset` against the bare bridge repo — the local analog of your GitHub remote. `reset` closes the bridge: clears its history and the container's clone (start a new one with a fresh `seed`) |
...
```bash
sim/omp-remote "Fix the failing test in src/auth.ts"
```

**Step 0 — Client bootstraps.**
python3 script starts. Reads the token from `sim/run/token` (or `OMP_REMOTE_TOKEN`), base URL from `OMP_REMOTE_URL` (default `http://localhost:8080`), session pointer from `~/.local/state/omp-remote/session.json`.
...
- **2c Git-bridge mode:** `POST /v1/sessions {gitBridge: true}`. Wrapper pins `cwd = /workspaces/git/project` and ensures it is a clone of the bare bridge repo (`ensureGitWorkspace()`), then proceeds as a normal session. Files move between this clone and your machine only as commits (`--sync` and `sim/git-bridge`, §11).
...
**Step 6 — Persistence.**
omp appends every transcript entry to `/state/agent/sessions/--workspaces-<uuid>--/<timestamp>_<sessionId>.jsonl` — on the bind mount, i.e. `sim/run/state/...` on the host. This file is what makes resume possible.
...
| Token rotation (each `simulate.sh` run) | n/a | n/a | n/a | client re-reads `sim/run/token` every call |
...
- **Auth:** every `/v1/*` route requires `Authorization: Bearer <token>`; `/healthz` is open (probe-style). Token is 24 random hex bytes, stored in `sim/run/token`.
...
- **Known caveats (inherited, deliberate):** workspaces may contain a `.env` that omp loads at boot; the container's `agent.db` holds real provider credentials imported from the host — treat `sim/run/state/` as secret. The container runs as your host UID with `HOME=/state`, so agent-created files are user-owned and directly removable from the host; legacy root-owned files (from deployments before 2026-09-07) are cleared by the UI's remove fallback (`docker exec … rm -rf`) or a one-time `chown` during `simulate.sh`.
...
sim/omp-remote --status         # client session pointer + live API state
```
...
- **Credential import (each deploy):** `sqlite3 ~/.omp/agent/agent.db ".backup …"` (consistent snapshot, WAL included) + copies of `config.yml` / `models.yml` / `models.db` into `sim/run/state/.omp/agent/`. Skip with `IMPORT_HOST_OMP_CONFIG=0`.
...
| `No model selected` at prompt | credential import not visible to the container (mount must map `sim/run/state/.omp` → `$HOME/.omp`, now `/state`) — fixed in current `simulate.sh` |
| `curl: (18)` + giant junk transcript | provider/model flake → retry (script does this); see `sim/run/smoke.sse` |
| 409 on every prompt | earlier turn still running → `sim/omp-remote --abort` |
...
| Bearer token in `sim/run/token` | IAP / OIDC id tokens |
| Host `~/.omp` import at deploy time | Secret Manager injection / `auth-broker` vault |
| Bind mounts (`sim/run/state`, `sim/run/workspaces`) | Persistent disk / Filestore / GCS |
...
| `sim/git-bridge seed\|push\|pull\|log\|reset <dir>` | host helper | publish your real project directory to the bridge; push local edits; pull container edits; inspect history; `reset` closes the current bridge (clears bridge history + the container clone at `/workspaces/git`) |
| `sim/run/git-bridge` → `/git-bridge` | deployment | the bare repo volume (wiped by `--clean`, like all state) |

**Workflow (verified end to end, both directions):**

```bash
# one-time: publish your project as the bridge's main branch
sim/git-bridge seed ~/projects/myapp

# container -> host: agent edits its clone, sync pushes, you pull
sim/omp-remote --new
sim/omp-remote --git "Add input validation to src/auth.ts"
sim/omp-remote --sync                        # commit + push to the bridge
sim/git-bridge pull ~/projects/myapp         # your local copy fast-forwards

# host -> container: your edits reach the agent
sim/git-bridge push ~/projects/myapp "wip: notes"
sim/omp-remote --sync                        # pull --rebase inside the container
sim/omp-remote --git "Review the latest changes in the working tree"
```
...
browser ──cookie auth──▶ ui backend (host, :8090, Node ≥20, zero deps)
                           │ holds sim/run/token; seeds workspaces (host fs + git)
                           ▼
...
**Run:** `node sim/ui/server.mjs` → open the printed URL → paste the printed token. The backend re-reads `sim/run/token` on every request, so `simulate.sh` redeploys need no UI restart.
